...
module.exports = {
    ...
    data: new SlashCommandBuilder()
        .setName('bchattype')
        .setDescription('Set the chat type your realm uses for chat relay')
        .addStringOption(option =>
            option.setName('type')
                .setDescription('Type of chat your realm has')
                .setRequired(true)
                .addChoices(
                    { name: 'Normal', value: 'Normal' },
                    { name: 'Tellraw', value: 'Tellraw' }
                )),
...
**__ Realms Updated __**
            `)
                                    ...
                                    const updatePromises = NonEmptySlots.slice(0, 5).map(async (Realm) => {
                                        ...
                                        if (configExists) {
                                            ...
                                            EmbedUpdate.addFields({
                                                ...
                                                value: `${end} Chat Type: \`${type}\``
                                            });
                                            ConfigUpdate.set("chattype", type);
                                            ConfigUpdate.save();
                                        } else {
                                            EmbedUpdate.addFields({
                                                name: `${denied} Realm Name: ${Realm.name}`,
                                                value: `${end} \`Could Not Update Chat Type\``
                                            });
                                        }
                                    });
                                    ...
                                    ConfigEditor.set("chattype", type);
                                    ConfigEditor.save();
