...
module.exports = {
    ...
    data: new SlashCommandBuilder()
        .setName('bglobal')
        .setDescription('Change wheither use Cosmos Global DB or not')
        .addBooleanOption(option =>
            option.setName('type')
                .setDescription('Activate or Deactivate Cosmos Global Ban')
                .setRequired(true)),
...
*Select Which Realm You Want To Enable/Disable Cosmos Global Ban!*
                `)
...
**__ Realms Updated __**
            `)
                                    ...
                                    const updatePromises = NonEmptySlots.slice(0, 5).map(async (Realm) => {
                                        ...
                                        if (configExists) {
                                            ...
                                            EmbedUpdate.addFields({
                                                ...
                                                value: `${end} Global Ban: \`${type}\``
                                            });
                                            ConfigUpdate.set("globalStatus", type);
                                            ConfigUpdate.save();
                                        } else {
                                            EmbedUpdate.addFields({
                                                name: `${denied} Realm Name: ${Realm.name}`,
                                                value: `${end} \`Could Not Update Global Ban\``
                                            });
                                        }
                                    });
                                    ...
                                    ConfigEditor.set("globalStatus", type);
                                    ConfigEditor.save();
