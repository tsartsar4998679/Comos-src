...
module.exports = {
    ...
    data: new SlashCommandBuilder()
        .setName('bminfollowers')
        .setDescription('Set a minimum followers required for players to join the realm')
...
**__ Realms Updated __**
            `)
                                    ...
                                    const updatePromises = NonEmptySlots.slice(0, 5).map(async (Realm) => {
                                        ...
                                        if (configExists) {
                                            ...
                                            EmbedUpdate.addFields({
                                                ...
                                                value: `${end} Followers: \`${amount}\``
                                            });
                                            ConfigUpdate.set("Followers", amount);
                                            ConfigUpdate.save();
                                        } else {
                                            EmbedUpdate.addFields({
                                                name: `${denied} Realm Name: ${Realm.name}`,
                                                value: `${end} \`Could Not Update Followers Amount\``
                                            });
                                        }
                                    });
                                    ...
                                    ConfigEditor.set("Followers", amount);
                                    ConfigEditor.save();
