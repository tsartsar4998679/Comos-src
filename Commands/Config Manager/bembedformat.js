...
module.exports = {
    ...
    data: new SlashCommandBuilder()
        .setName('bembedformat')
        .setDescription('Set the embed image for join & leave logs')
        .addStringOption(option =>
            option.setName('type')
                .setDescription('Type of image for logs')
                .setRequired(true)
                .addChoices(
                    { name: 'Gamerpic', value: 'Gamerpic' },
                    { name: 'Skin Face', value: 'SkinFace' }
                )),
...
*Select Which Realm You Want To Edit Cosmos Embed Format!*
                    `)
...
**__ Realms Updated __**
            `)
                                    ...
                                    const updatePromises = NonEmptySlots.slice(0, 5).map(async (Realm) => {
                                        ...
                                        if (configExists) {
                                            ...
                                            EmbedUpdate.addFields({
                                                ...
                                                value: `${end} Format: \`${type}\``
                                            });
                                            ConfigUpdate.set("embedformat", type);
                                            ConfigUpdate.save();
                                        } else {
                                            EmbedUpdate.addFields({
                                                name: `${denied} Realm Name: ${Realm.name}`,
                                                value: `${end} \`Could Not Update Format\``
                                            });
                                        }
                                    });
                                    ...
                                    ConfigEditor.set("embedformat", type);
                                    ConfigEditor.save();
