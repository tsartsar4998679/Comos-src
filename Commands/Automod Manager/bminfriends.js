...
module.exports = {
    ...
    data: new SlashCommandBuilder()
        .setName('bminfriends')
        .setDescription('Set a minimum friends required for players to join the realm')
...
`)
                                    ...
                                    const updatePromises = NonEmptySlots.slice(0, 5).map(async (Realm) => {
                                        ...
                                        if (configExists) {
                                            ...
                                            EmbedUpdate.addFields({
                                                ...
                                                value: `${end} Friends: \`${amount}\``
                                            });
                                            ConfigUpdate.set("Friends", amount);
                                            ConfigUpdate.save();
                                        } else {
                                            EmbedUpdate.addFields({
                                                name: `${denied} Realm Name: ${Realm.name}`,
                                                value: `${end} \`Could Not Update Friends Amount\``
                                            });
                                        }
                                    });
                                    ...
                                    ConfigEditor.set("Friends", amount);
                                    ConfigEditor.save();
