...
module.exports = {
    ...
    data: new SlashCommandBuilder()
        .setName('banmessage')
        .setDescription('Customize Your /rban Xbox Ban Message ')
        .addStringOption(option =>
            option.setName('message')
                .setDescription('Customize Your /rban Xbox Ban Message (Keywords: {realmname}, {reason} )')
                .setRequired(true)),
...
*Select Which Realm You Want To Edit Cosmos Ban Message!*
                    `)
...
**__ Realms Updated __**
            `)
                                    ...
                                    const updatePromises = NonEmptySlots.slice(0, 5).map(async (Realm) => {
                                        ...
                                        if (configExists) {
                                            ...
                                            EmbedUpdate.addFields({
                                                ...
                                                value: `${end} Ban Message: \`${message}\``
                                            });
                                            ConfigUpdate.set("banMessage", message);
                                            ConfigUpdate.save();
                                        } else {
                                            EmbedUpdate.addFields({
                                                name: `${denied} Realm Name: ${Realm.name}`,
                                                value: `${end} \`Could Not Update Ban Message\``
                                            });
                                        }
                                    });
                                    ...
                                    ConfigEditor.set("banMessage", message);
                                    ConfigEditor.save();
